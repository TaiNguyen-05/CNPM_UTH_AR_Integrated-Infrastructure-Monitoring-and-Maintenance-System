...
export const AuthView: React.FC<AuthViewProps> = ({
  ...
  // Function xử lý tài khoản Google (dùng chung cho cả Native Popup SDK và Form Modal)
  const processGoogleAccount = async (cleanEmail: string, cleanName: string, avatarUrl?: string) => {
    setIsLoading(true);
    setGoogleNotice(null);

    try {
      const { arImmsApi } = await import('../services/api');
  ...
  // Khởi tạo Google Identity Services SDK và render button chính thức
  React.useEffect(() => {
    try {
      ...
      if (g && g.accounts && g.accounts.id) {
        ...
        const btnContainer = document.getElementById('google-signin-btn-wrapper');
        if (btnContainer) {
  ...
  // Trigger Google Login (Bấm nút Google)
      ...
      if (g && g.accounts && g.accounts.id) {
        g.accounts.id.prompt((notification: any) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // Nếu trình duyệt chặn popup / One Tap -> Mở Modal nhập nhanh
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Lưu tài khoản vào Backend Database
      import('../services/api').then(async ({ arImmsApi }) => {
  ...
  return (
    <div className="min-h-screen bg-[#080b0e] text-slate-200 flex flex-col justify-between font-sans selection:bg-[#f59e0b] selection:text-[#080b0e] relative overflow-hidden">
          ...
          <div className="lg:col-span-7 p-6 sm:p-8 flex flex-col justify-center bg-black/20 backdrop-blur-sm">
            ...
            <div className="flex border border-white/15 bg-black/40 p-1 mb-6 font-mono text-xs rounded-xl shadow-inner">
              <button
                ...
                className={`flex-1 py-2 text-center transition-all cursor-pointer uppercase tracking-wider font-bold rounded-lg ${
                  authMode === 'login' 
                    ? 'bg-[#38bdf8] text-[#080b0e] shadow-md' 
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-black/35 backdrop-blur-xs border border-white/20 text-white font-mono text-xs focus:border-[#38bdf8] focus:bg-black/50 focus:outline-none rounded-lg placeholder:text-slate-500 shadow-inner"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full keycap-glow bg-[#38bdf8] hover:bg-[#00f0ff] text-[#080b0e] py-3 text-xs font-mono font-bold uppercase tracking-widest transition-all cursor-pointer active:scale-95 disabled:opacity-50 mt-2 shadow-xl rounded-lg"
                >
                ...
                <div className="flex flex-col items-center justify-center w-full gap-2 pt-1">
                  <div id="google-signin-btn-wrapper" className="w-full flex justify-center min-h-[44px]"></div>
                  
                  {/* Fallback button if Google SDK is loading or popup is blocked */}
                  <button
                    type="button"
                    onClick={handleTriggerGoogleLogin}
                    className="w-full flex items-center justify-center gap-2.5 py-2 px-4 bg-black/35 hover:bg-black/60 backdrop-blur-xs border border-white/20 hover:border-[#38bdf8]/60 text-slate-200 hover:text-white text-xs font-mono font-medium transition-all cursor-pointer shadow-md rounded-lg"
                  >
                    ...
                    <span>Mở Hộp Thoại Google SSO</span>
                  </button>
                </div>
              </form>
            )}
            ...
            {authMode === 'register' && (
              <form onSubmit={handleRegisterSubmit} className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-mono text-slate-300 mb-1 uppercase font-bold">Họ & Tên</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full px-3 py-2 bg-black/35 backdrop-blur-xs border border-white/20 text-white font-mono text-xs focus:border-[#38bdf8] focus:outline-none rounded-lg"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-mono text-slate-300 mb-1 uppercase font-bold">Email Công Vụ</label>
                    <input
                      type="email"
                      required
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-mono text-slate-300 mb-1 uppercase font-bold">Vai Trò Đăng Ký</label>
                    <select
                      value={regRole}
                  ...
                  <div>
                    <label className="block text-[10px] font-mono text-slate-300 mb-1 uppercase font-bold">Mã Nhân Viên (Tùy chọn)</label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-mono text-slate-300 mb-1 uppercase font-bold">Mật Khẩu</label>
                    <input
                      type="password"
                      required
                      placeholder="••••••••"
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="border border-[#222c37] bg-[#0c1017] max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center pb-3 border-b border-[#222c37]">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs font-bold uppercase text-white">Xác Thực Google OAuth SSO</span>
              </div>
              <button 
                onClick={() => setShowGoogleModal(false)}
                className="text-slate-500 hover:text-white p-1 text-sm cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            {googleNotice ? (
              <div className="space-y-4">
                <div className="p-3 bg-sky-950/40 border border-sky-500/40 text-sky-200 text-xs font-mono leading-relaxed">
                  {googleNotice}
                </div>
                <button
                  onClick={() => setShowGoogleModal(false)}
                  className="w-full bg-[#38bdf8] text-[#080b0e] py-2.5 text-xs font-mono font-bold uppercase hover:bg-[#00f0ff] transition-colors cursor-pointer"
                >
                  Đã Hiểu & Đóng
                </button>
              </div>
              ...
              <form onSubmit={handleGoogleAuthSubmit} className="space-y-3.5">
                ...
                <div>
                  <label className="block text-[11px] font-mono text-slate-400 mb-1 uppercase">Địa Chỉ Gmail (*)</label>
                  <input
                    type="email"
                    required
                    ...
                    className="w-full px-3 py-2 bg-[#161d24] border border-[#222c37] text-white font-mono text-xs focus:border-[#38bdf8] focus:outline-none"
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-mono text-slate-400 mb-1 uppercase">Họ Và Tên (Kỹ thuật viên)</label>
                  <input
                    type="text"
                ...
                <div className="pt-2 flex gap-2">
                  <button
                    type="button"
                    onClick={() => setShowGoogleModal(false)}
                    className="flex-1 bg-[#161d24] border border-[#222c37] text-slate-400 py-2.5 text-xs font-mono uppercase hover:text-white cursor-pointer"
                  >
                    Hủy
                  </button>
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="flex-1 bg-[#38bdf8] hover:bg-[#00f0ff] text-[#080b0e] py-2.5 text-xs font-mono font-bold uppercase transition-all cursor-pointer shadow-md disabled:opacity-50"
                  >
